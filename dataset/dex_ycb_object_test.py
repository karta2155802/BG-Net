...
from utils.vis import vis_keypoints_with_skeleton, vis_keypoints, vis_bbox
...
class dex_ycb(data.Dataset):
    def __init__(self, dataset_root, obj_model_root, mode, inp_res=256, max_rot=np.pi, scale_jittering=0.2, center_jittering=0.1,
        ...
        #object information
        self.obj_mesh, self.obj_vertices_dict, self.obj_faces_dict, ori_model = dex_ycb_util.load_objects_dex_ycb(obj_model_root)
        ...
        for key in self.obj_mesh:
            mesh = ori_model[key]
            points = self.obj_bbox3d[key]
            colors = [[0, 0, 255] for i in range(len(lines))]
            ...
            o3d.visualization.draw_geometries([line_set, mesh, pcd], width=720, height=720)'''
    ...
    def data_aug(self, img, mano_param, joints_uv, K, gray, p2d):
        ...
        bbox_hand = dataset_util.get_bbox_joints(joints_uv, bbox_factor=self.hand_factor)
        joints_uv = dataset_util.normalize_joints(joints_uv, bbox_hand)

        # get obj bbox and normalize landmarks to [0,1]
        bbox_obj = dataset_util.get_bbox_joints(p2d, bbox_factor=self.obj_factor)
    ...
    def data_crop(self, img, mano_param, K, hand_joints_2d, p2d):
        crop_hand = dataset_util.get_bbox_joints(hand_joints_2d, bbox_factor=1.5)
        crop_obj = dataset_util.get_bbox_joints(p2d, bbox_factor=1.5)
        bbox_hand = dataset_util.get_bbox_joints(hand_joints_2d, bbox_factor=self.hand_factor)
        bbox_obj = dataset_util.get_bbox_joints(p2d, bbox_factor=self.obj_factor)
        center, scale = dataset_util.fuse_bbox(crop_hand, crop_obj, img.size)
        ...
        ii = np.array(img)
        '''lines = [[0,2], [1,3], [2,3], [0,1], [4,6], [5,7], [6,7], [4,5], [2,6], [0,4], [3,7], [1,5]]
        vv = vis_keypoints_with_skeleton(ii, p2d, lines, radius=3, color=(255, 255, 0)).astype(np.uint8)
        vv = vis_keypoints(vv, p2d, radius=3, color=(0, 255, ))
        vv = vis_bbox(vv, bbox_obj, thickness=2, color=(0,0,255))'''
        
        '''j2d = dataset_util.transform_coords(hand_joints_2d, affinetrans)
        ...
        vv = vis_bbox(vv, bbox_hand, thickness=2, color=(255,0,0))
        vv_out = Image.fromarray(vv)
        vv_out.save(os.path.join("exp-results", "bbox.png"))
        plt.imshow(vv)
        plt.show()'''

        K_no_trans = K
        K = affinetrans.dot(K)
        return img, mano_param, affinetrans, bbox_hand, bbox_obj, K, K_no_trans
    ...
    def __getitem__(self, idx):
        ...
        subject, seq, videoID, id = sample_info["color_file"].split('/')
        id = id[-10:-4]
        seg_path = os.path.join(self.root, "train_segLabel", subject, seq, videoID, id + ".png")
        sample["img_name"] = subject + "_" + seq + "_" + videoID + "_" + id
        ...
        if self.mode == "train":
            ...
            #crop
            img, mano_param, affinetrans, bbox_hand, bbox_obj, K, K_no_trans = self.data_crop(img, mano_param, K, joints_uv, p2d)
