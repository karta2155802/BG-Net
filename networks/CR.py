...
class Transformer(nn.Module):
    def __init__(self, inp_res=32, dim=256, depth=1, num_heads=1, mlp_ratio=4.):
        ...
        for _ in range(depth):
            self.layers.append(Block(dim=dim, inp_res=inp_res, num_heads=num_heads, mlp_ratio=mlp_ratio))
...
class Block(nn.Module):
    ...
    def __init__(self, dim, inp_res, num_heads, mlp_ratio=4., act_layer=nn.GELU, norm_layer=nn.LayerNorm):
        super().__init__()
    ...
    def forward(self, query, key, query_embed=None, key_embed=None):
        ...
        attn, attn_score = self.attn(query=q, key=k, value=v)
        query = query + attn
