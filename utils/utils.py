...
class Monitor:
    ...
    def log_train(self, errors):
        log_errors(errors, self.train_path)
...
def log_errors(errors, log_path=None):
    with open(log_path, "a") as log_file:
...
def load_checkpoint(model, resume_path, optimizer=None, scheduler=None, strict=True, device=None):
...
def get_dataset(args, mode):
    ...
    dataset = HO3D(dataset_root=args.HO3D_root, obj_model_root=args.obj_model_root, mode=mode, inp_res=args.inp_res)
    return dataset

def get_dex_ycb_dataset(args, mode):
    from dataset.dex_ycb_object_test import dex_ycb
    dataset = dex_ycb(args.dex_ycb_root, obj_model_root=args.obj_model_root, mode=mode, inp_res=args.inp_res)
    return dataset
