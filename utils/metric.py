...
def get_vis_mesh(pred_pose, gt_pose, obj_mesh):
    pred = np.matmul(pred_pose[:3, 0:3], obj_mesh.T) + pred_pose[:3, 3].reshape(-1, 1)
    gt = np.matmul(gt_pose[:3, 0:3], obj_mesh.T) + gt_pose[:3, 3].reshape(-1, 1)  # (3,N)
    return pred.T, gt.T
...
def eval_batch_obj(batch_output, obj_bbox,
                   ...
                   cam_intr, REP_res_dic, ADD_res_dic, bestCnt=1, batch_affinetrans=None, batch_hand_type=None):
    ...
    pred_obj, gt_obj = get_vis_mesh(pred_pose, obj_pose[-1], obj_vertices_dict[cls])
    return REP_res_dic, ADD_res_dic, pred_obj, gt_obj, p2d
...
def eval_object_pose(REP_res_dic, ADD_res_dic, diameter_dic, outpath, unseen_objects=[], epoch=None):
    ...
    for k in ADD_res_dic.keys():
        ...
        if not np.isnan(ADD_10[k]) and k not in unseen_objects:
            all_ADD_10.append(ADD_10[k])
    ...
    print("ADD_10_avg:", np.array(all_ADD_10).mean(), file=log_file, end="\n\n")
    log_file.close()
    return ADD_10, REP_5
...
def eval_hand(preds_joint, gts_hand_type, gts_joints_coord_cam, hand_eval_result):
    sample_num = len(preds_joint)
    for n in range (sample_num):
        pred_joint = preds_joint[n]
        gt_hand_type = gts_hand_type[n]
        gt_joints_coord_cam = gts_joints_coord_cam[n].cpu().numpy()
