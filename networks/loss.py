...
class ObjectLoss:
    def __init__(self, obj_reg_loss_weight, obj_conf_loss_weight=None, obj_loss_func=None):
        ...
        self.obj_border_loss_weight = obj_conf_loss_weight / 5
    ...
    def compute_border_loss(self, obj_pred):
        B = obj_pred[0].shape[0]
        device = obj_pred[0].device
        top_left_gt = torch.zeros(B, 2).to(device)
        bottom_right_gt = torch.ones(B, 2).to(device)

        reg_px = obj_pred[0].view(B, -1, 21)
        reg_py = obj_pred[1].view(B, -1, 21)
        reg_conf = obj_pred[2].view(B, -1, 21) # Bx1024x21
        
        val, idx = torch.topk(reg_conf, k=1, dim=1)
        xs = reg_px.gather(dim=1, index=idx) # Bx1x21
        ys = reg_py.gather(dim=1, index=idx)

        p2d = torch.cat([xs, ys], dim=1) # Bx2x21
        top_left = p2d.min(dim=2)[0] # Bx2
        bottom_right = p2d.max(dim=2)[0]
        
        top_left_loss = self.obj_border_loss_weight  * self.obj_loss_func(top_left, top_left_gt)
        bottom_right_loss = self.obj_border_loss_weight  * self.obj_loss_func(bottom_right, bottom_right_gt)
