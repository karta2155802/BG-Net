...
class HO3D(data.Dataset):
    def __init__(self, dataset_root, obj_model_root, mode="evaluation", inp_res=512,
        ...
        # object informations
        self.obj_mesh, self.obj_vertices_dict, self.obj_faces_dict = ho3d_util.load_objects_HO3D(obj_model_root)
        ...
        if self.mode == "train":
            ...
            self.train_seg_root = os.path.join(dataset_root, "train_segLabel")
            ...
            self.set_list  = []
            #data_ho3d = []
            with open(os.path.join(dataset_root, 'ho3d_train_data.json'), 'r') as f:
    ...
    def data_crop(self, img, K, bbox_hand, p2d):
        ...
        crop_obj = dataset_util.get_bbox_joints(p2d, bbox_factor=1.5)
        bbox_hand = dataset_util.get_bbox_joints(bbox_hand.reshape(2, 2), bbox_factor=self.hand_factor)
        bbox_obj = dataset_util.get_bbox_joints(p2d, bbox_factor=self.obj_factor)
        center, scale = dataset_util.fuse_bbox(crop_hand, crop_obj, img.size)
        ...
        vv = vis_keypoints(vv, p2d, radius=3, color=(0, 255, 0))
        ...
        K_no_trans = K
        K = affinetrans.dot(K)
        return img, K, bbox_hand, bbox_obj, affinetrans, K_no_trans
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'train':
            ...
            img, K, bbox_hand, bbox_obj, affinetrans, K_no_trans = self.data_crop(img, K, bbox_hand, p2d)
            sample["img"] = functional.to_tensor(img)
